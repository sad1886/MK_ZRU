...
void Razryd_NVAB_noCAN (void)													/* _Р_А_З_Р_Я_Д___Н_В_А_Б_ по проводным командам */
{
	...
	case bUabCheck:
			if	((aU_zru >= 88) && (!bitNotRaz))	{																// Напряжение АБ достигло 88В И нет запрета разряда ЗРП = 0	
				StepAlgortmRazr = bWaitOtkl_ZaprRaz;														// Переход на начало отключение запрета разряда
			}	
			else	{																														//
	...
	case bTst_I_Comp2:
		...
		if (!bPauza_R) {
			...
			StepAlgortmRazr = bWaitPause;															// Переход на ожидание подъёма напряжения АБ до 80В
			LimsCount_R = vsCount20;	 sCount_R=0;	bPauza_R=1;								// Сброс счётчика , включене паузы 20 сек
		}	
		break;
	...
	case bWaitPause:
		...
		if (!bPauza_R) {																							// Если дождались 
			StepAlgortmRazr = bUabCheck;																	// Переход на контроль Uаб
		}
		break;
