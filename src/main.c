...
//-------------------------------------------------------------------------------------------------------------------------
unsigned char updateUAB1,																			// Флаг обновления данных от МУК1 БЭ пакет №1
							updateUAB2,																			// Флаг обновления данных от МУК2 БЭ пакет №1
							updateUAk1,																			// Флаг обновления данных от МУК1 БЭ пакет №2
							updateUAk2,																			// Флаг обновления данных от МУК2 БЭ пакет №2
							bMakeAK;																				// Флаг: расчитываем телеметрию на значениях UAB1 и UAB2 МУК3

union uBytesFloat16 UAB[6];
union uBytesFloat16 SumUAk1;
union uBytesFloat16 SumUAk2;
...
*/
	...
	//.............................................................Телеметрия датчиков
		...
		for (ak=0; ak < nfVdatch; ak++)	{
			for (j=0; j<2; j++)	{
				fVdatch[ak].b[j] = Reciev_CanDatch[iMUK_ZRU][fr].b[j+adj];								// 
			}
			switch (fr)		{
		...
		iVdatch[0] = Reciev_CanDatch[iMUK_ZRU][0].b[4];																// номер ДД с максимальным отклонением
		iVdatch[1] = Reciev_CanDatch[iMUK_ZRU][0].b[7];																// номер ДД с максимальным значением
		iVdatch[2] = Reciev_CanDatch[iMUK_ZRU][1].b[2];																// номер ДД с минимальным значением
		iVdatch[3] = Reciev_CanDatch[iMUK_ZRU][1].b[7];																// номер ДТ с максимальным отклонением
		iVdatch[4] = Reciev_CanDatch[iMUK_ZRU][2].b[2];																// номер ДТ с максимальным значением
		iVdatch[5] = Reciev_CanDatch[iMUK_ZRU][2].b[5];																// номер ДТ с минимальным значением
		iVdatch[6] = Reciev_CanDatch[iMUK_ZRU][9].b[2];																// состояние РС НВАБ
	...
	//...........................................................Телеметрия АБ
		...
		for (ak=0; ak < nfV_AB; ak++)	{
			for (j=0; j<2; j++)	{
				fV_AB[ak].b[j] = Reciev_CanAB[iMUK_ZRU][fr].b[j+adj];											// 
			}
			switch (fr)		{
			case 0:	if (ak==0) 	adj = 2;
							if (ak==1) 	adj = 6;
		...
		iV_AB[0] = Reciev_CanAB[iMUK_ZRU][0].b[4];																		// 5	номер АЭ с максимальным отклонением
		iV_AB[1] = Reciev_CanAB[iMUK_ZRU][0].b[5];																		// 6	номер АЭ с максим. значением напряжения
		iV_AB[2] = Reciev_CanAB[iMUK_ZRU][19].b[4];																		// 5	номер АЭ с миним. значением напряжения
...
{		int ii;
	...
	case bWaitVkl_Tst_Zarayd:		
		...
		if (!bPauza) {
			...
			//if	(aI_zar < 50)	{	stat2[iMUK_ZRU] &= ~errNoVklZar;							// Отладочная заглушка, Собщение "Не включился Заряд АБ"=0
				stat4[iMUK_ZRU] &= ~bready7;																			// Сброс флага Прекращаем заряд
	...
	case bVkl_Tst_Zarayd_On:			
		...
		if (!bPauza) {
			if	((T >= Tn3)||																									// (T>Tn3)||
					 ...
					 ((stat4[iMUK_ZRU2] & bready7)&&															// Заряд завершён
					 ( stat4[iMUK_ZRU3] & bready7)) 	
					)
			{			 
				if ((stat4[iMUK_ZRU2] & bready7)&&															// Заряд завершён
						(stat4[iMUK_ZRU3] & bready7)) 	
				{			
					...
					if (P >= Pv)	{																								// "Превышение допустимой температуры НВАБ"
						StepAlgortm = bVkl_ZaprZarayd;	}														// Для первого заряда StepNext = bOtkl_Zarayd. Для второго StepNext = bCmp_T_Tv_P_Pn
					else	{
						StepAlgortm = bWeitMKs;					}														// Ожидание завершения заряда на других МК
				}
			}	
	...
	// ========== Ожидание завершения заряда на других МК....................................................................7a
	case bWeitMKs:

		stat4[iMUK_ZRU] |= bready7;																					// Установили флаг Прекращаем заряд
		...
		if ((stat4[iMUK_ZRU2] & bready7)||																	// Заряд завершён
			...
			if ((stat3[iMUK_ZRU2] & bZaprZar)||																// включен запрет Заряда?
				  (stat3[iMUK_ZRU3] & bZaprZar))	{
				StepAlgortm = bVkl_ZaprZarayd;		
			}	
			else	{
				StepAlgortm = bWeitMKs;																				// Ожидание завершения заряда на других МК
			}	
...
*/
//-------------------------------------------------------------------------------------------------------------------------
unsigned char fByte3(float fv, int ind)
...
{ 
/*	
	//Коррекция напряжения 72-го АК АБ ДЛЯ БЭ
	if (mode_Razryad) {	fV_AB[76].Fdata += aI_razr*0.0;	}
	if (mode_Zaryad)  {	fV_AB[76].Fdata -= aI_zar *0.0;	}
*/
	if (stat1[iMUK_ZRU] & bPC)		  {	fV_AB[76].Fdata += 0.1;	}						//Коррекция напряжения 72-го АК АБ
...
//-------------------------------------------------------------------------------------------------------------------------
unsigned char fByte7(float fv, int ind)
{	unsigned char b;	int tmp;
	
	if (fv >= x0_p7[ind])		{
		tmp = (int) ((fv-x0_p7[ind])/z_p7[ind]+0.5);
		if (tmp>255)	b = 255;	else	b = tmp;	}	
	else	b = 0;
...
//	PackRs7[7] = (tMin - x0_p7[2])/z_p7[2];																// Текущее время от начала этапа
	...
	PackRs7[5] = fByte7(W_raz,0);
	PackRs7[6] = fByte7(C_raz,1);
	PackRs7[7] = fByte7(tMin, 2);
...
void Razryd_NVAB_noCAN (void)													/* _Р_А_З_Р_Я_Д___Н_В_А_Б_ по проводным командам */
{
	...
	// .......... Контроль Uаб .....................................................................................
	case bUabCheck:
			if	((vU_zru >= 88) && (!bitNotRaz))	{														// Напряжение АБ достигло 88В И нет запрета разряда ЗРП = 0	
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
void getUAB (int ind,int id)	{
		UAB[ind].b[0] = Reciev_CanDatch[id][7].b[6];									// 
		UAB[ind].b[1] = Reciev_CanDatch[id][7].b[7];									// Напряжение АБ1
		UAB[ind+1].b[0] = Reciev_CanDatch[id][8].b[0];								// 
		UAB[ind+1].b[1] = Reciev_CanDatch[id][8].b[1];								// Напряжение АБ2 
}	

//...........................................................Телеметрия АK
void getUAk1(void)	
{	int i,j,adj,fr;		union uBytesFloat16 Ak;
	
	SumUAk1.Fdata=0;
	fr = 1;		adj = 4;
	for (i=0; i < 72; i++)	{																				//
		for (j=0; j<2; j++)	{
			Ak.b[j] = Reciev_CanAB[0][fr].b[j+adj];											// 
		}
		SumUAk1.Fdata += Ak.Fdata;
		if (adj<6) adj += 2;	else	{	adj=0;	fr++;	}
	}
}
...
{	int i,j,adj,fr;		union uBytesFloat16 Ak;
	...
	for (i=0; i < 72; i++)	{																				//
		for (j=0; j<2; j++)	{
			Ak.b[j] = Reciev_CanAB[1][fr].b[j+adj];											// 
		}
		SumUAk2.Fdata += Ak.Fdata;
		if (adj<6) adj += 2;	else	{	adj=0;	fr++;	}
	}
}

//...........................................................
float ValueMiddle(void)
{	float x;	int j;
	// сортировка вставками
  for (i = 1; i < 6; ++i) {
    x = UAB[i].Fdata;
    j = i;
    while ((j > 0) && (UAB[j - 1].Fdata > x)) {
      UAB[j].Fdata = UAB[j - 1].Fdata;
      --j;
    }
    UAB[j].Fdata = x;
  }
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
// Только для nMUK3 ЗРУ)
void GetParametrsAk(void)															
{	float vMid;
		//.......................................................... Контроль обновления данных телеметрии.....................
			if ((nfRec_CanDatch1[0] == okFrameDatch)||
					(nfRec_CanDatch2[0] == okFrameDatch))	{														// Получены все фреймы пакета №1 от МК1
				getUAB (0,0);
				nfRec_CanDatch1[0] = 0;		nfRec_CanDatch2[0] = 0;										// Сброс битов получения фреймов
				updateUAB1 = 1;																											// Установить флаг "Обновление данных UАБ от МУК1 БЭ"
			}
			if ((nfRec_CanAK1[0] == okFrameAK)||
					(nfRec_CanAK2[0] == okFrameAK))				{														// Получены все фреймы пакета №2 от МК1
				getUAk1();																													// Получение суммарного по АК напряжения АБ от МК1
				nfRec_CanAK1[0] = 0;		nfRec_CanAK2[0] = 0;												// Сброс битов получения фреймов
				updateUAk1 = 1;																											// Установить флаг "Обновление данных UАК от МУК1 БЭ"
			}
		//..........................................................	
			if ((nfRec_CanDatch1[1] == okFrameDatch)||
					(nfRec_CanDatch2[1] == okFrameDatch))	{														// Получены все фреймы пакета №1 от МК2
				getUAB (2,1);
				nfRec_CanDatch1[1] = 0;	nfRec_CanDatch2[1] = 0;											// Сброс битов получения фреймов
				updateUAB2 = 1;																											// Установить флаг "Обновление данных UАБ от МУК2 БЭ"
			}
			if ((nfRec_CanAK1[1] == okFrameAK)||
					(nfRec_CanAK2[1] == okFrameAK))				{														// Получены все фреймы пакета №2 от МК2
				getUAk2();																													// Получение суммарного по АК напряжения АБ от МК2
				nfRec_CanAK1[1] = 0;		nfRec_CanAK2[1] = 0;														// Сброс битов получения фреймов
				updateUAk2 = 1;																											// Установить флаг "Обновление данных UАК от МУК2 БЭ"
			}
		//..........................................................	
			if ((nfRec_CanDatch1[2] == okFrameDatch)||
					(nfRec_CanDatch2[2] == okFrameDatch))	{														// Получены все фреймы пакета №1 от МК3
				getUAB (4,2);
				nfRec_CanDatch1[2] = 0;	nfRec_CanDatch2[2] = 0;											// Сброс битов получения фреймов
			}
		
		bMakeAK = 0;
		//..............................................................................................................................
		if	(updateUAB1 & updateUAk1)	{
			if (updateUAB2 & updateUAk2)	{																				// Данные получены от МК1 и МК2
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
// Расчёт массива напряжений аккумуляторов (АК)
void RaschotArrayAE_MUK3 (void)
{		unsigned char	ibias, imaxU, iminU;
		union uBytesFloat16 AEsr, pki[2], maxU, minU, dU_AE_IsmAE;
	...
	pki[0].Fdata = AEsr.Fdata;																								// Вычисление среднего значения напряжения на всех АЭ
	pki[1].Fdata = 0;																													// Макс отклон от среднего
	ibias = 0;																																// Номера АЭ макс отклон от среднего
	imaxU = 0;																																// Номер с максимальным значением напряжения
	maxU.Fdata = AEsr.Fdata;																									// макс значение напряжения
	minU.Fdata = AEsr.Fdata;																									// мин значение напряжения
	dU_AE_IsmAE.Fdata = 0;																										// Разница напряжений между измерительными и неизмерительными АK
	// Заполнение значений напряжений АК
	for (i=0; i<nAllAE; i++)	{
		AK[i].Fdata = AEsr.Fdata;
	}
	iminU = 0;																																// Номер с минимальным значением напряжения

	//...................................................................
	// Подготовка массива байт для телеметрии АБ
	j=0;
	for (i=0; i < 2; i++)	{	
	...
	// Подготовка информации по АK
	for (i=0; i < nAllAE; i++)	{																							// Массив значений напряжений АK, 72
		dataCAN[j] = AK[i].b[0];	j++;
		dataCAN[j] = AK[i].b[1];	j++;
	}																																					// 144+14=158
...
//	Ports_Init_Tst(); 																										// Инициализация портов для отладочной платы
	...
	while (1)												
	{
		...
		// Только для МУК3 ####################################################################################################
		if (nMUK_ZRU==nMUK3_ZRU)	{
			if ((updateD1)&&(nfRec_CanDatch1[iMUK_ZRU] == okFrameDatch))	{			// Получены все фреймы на чётном цикле
				// МУК3 получает телеметрию АК от МУК1 или МУК2 ...................................................................
				GetParametrsAk();
				// Расчёт значений напряжений для АК МУК3 на основе УАБ1 и УАБ2 МУК3 ..............................................
				if (bMakeAK)	{		RaschotArrayAE_MUK3();		}		
				nfRec_CanAK1[iMUK_ZRU] = okFrameAK;
			}
		}
		// ####################################################################################################################
			
		//.......................................................... Контроль обновления данных телеметрии БЭ .................
		if (nfRec_CanDatch1[iMUK_ZRU] == okFrameDatch)	{											// Получены все фреймы
			add_nbuf = 0;
			CAN_SendConf_1(1);																									// Подтверждение: Даные от МУК БЭ получены
			nfRec_CanDatch1[iMUK_ZRU] = 0;																			// Сброс битов получения фреймов
			updateD1 = 1;																												// Установить флаг "Обновление данных от МУК БЭ"
		}
		if (nfRec_CanAK1[iMUK_ZRU] == okFrameAK)	{														// Получены все фреймы
			add_nbuf = 1;
			CAN_SendConf_1(2);																									// Подтверждение: Даные от МУК БЭ получены
			nfRec_CanAK1[iMUK_ZRU] = 0;																					// Сброс битов получения фреймов
			updateD2 = 1;																												// Установить флаг "Обновление данных от МУК БЭ"
		}
		
		if (nfRec_CanDatch2[iMUK_ZRU] == okFrameDatch)	{											// Получены все фреймы
			add_nbuf = 0;
			CAN_SendConf_2(1);																									// Подтверждение: Даные от МУК БЭ получены
			nfRec_CanDatch2[iMUK_ZRU] = 0;																			// Сброс битов получения фреймов
			updateD1 = 1;																												// Установить флаг "Обновление данных от МУК БЭ"
		}
		if (nfRec_CanAK2[iMUK_ZRU] == okFrameAK)	{														// Получены все фреймы
			add_nbuf = 1;
			CAN_SendConf_2(2);																									// Подтверждение: Даные от МУК БЭ получены
			nfRec_CanAK2[iMUK_ZRU] = 0;																					// Сброс битов получения фреймов
			updateD2 = 1;																												// Установить флаг "Обновление данных от МУК БЭ"
		}
