...
volatile unsigned char iUst_mas[3];																	// Значения уставок всех трех МК
...
unsigned char mk_zru_con[3]={0,0,0};	//связь МК ЗРУ с другими МК ЗРУ

extern volatile unsigned char bRestData; 														// 1 - восстановить данные
...
//дельта времени, необходимая для расчета W и C, может меняться в зависимости от паузы между измерениями, по умолчанию соответствует дельте в 5 секунд
...
extern unsigned char cntIzmZar, cntIzmRaz;
...
{	
	...
	for (i=0; i <= 4; i++) 
	{
		if(nBadAK_DT[i] == 0) //находим первый неотказавший измерительный АК
		{
			Pmax = P_array[i];
			Pmin = P_array[i];
			break;							// прекращаем цикл (повторять нет смысла)
		}	
	}	
...
{	
	...
	tVir 	 = tVir_def;																										// Время выравнивания заряженности НВАБ при ТВЦ, в минутах 
...
void PauseOn(unsigned int p)
{
	LimsCount = p; 	//секунды
	sCount=0;	
	bPauza=1;				//Активация паузы 
}
...
{		
	int ii; //для микрозадержки

	switch (StepAlgortmTest)	// Переключатель состояний алгоритма ТВЦ
	{
	// .......... Инициализация ТВЦ .........................................................................................
	case st_t_InitTest:
	...
	case st_t_1_04:			
		...
		stat4[iMUK_ZRU] &= ~br1;																						  
		PauseOn(20);
		StepAlgortmTest = st_t_1_05;           
	break;
		
	// .......... Ожидание включения Тест Заряда ............................................................................
	case st_t_1_05:	
		
		if (!bPauza) {
			if	(aI_zar > aIporog)	{	
				stat2[iMUK_ZRU] &= ~errNoVklZar;				// Собщение "Не включился Заряд АБ"=0
				StepAlgortmTest = st_t_1_06;
			}
			else	{									
	...
	case st_t_1_08:		
	  ...
	  if ( (!bPauza) &&
		...
		{
			if	(aI_zar > aIporog)		
				stat2[iMUK_ZRU] |= errNoOtklZar;																// Собщение "Не отключился Заряд АБ"=1
			else								
				stat2[iMUK_ZRU] &= ~errNoOtklZar;																// Собщение "Не отключился Заряд АБ"=0
			StepAlgortmTest = st_t_2_01;																			
		}
		break;
	...
	case st_t_2_04:		
		...
		if (!bPauza) {
			if	((T >= Traz)||																									// Контроль температуры
					 (tstatTVC/60 >= tP_TVC)||																			
					 (Uab <= 72)||																									// напряжения АБ
					 (Umin_ak <= 0.1)||
					 ((stat4[iMUK_ZRU2] & br2) && (stat4[iMUK_ZRU3] & br2))
					)																															
			{			 
				if (T >= Traz)	
					stat3[iMUK_ZRU] |= errPrevDopustT;															// Превышение допустимой температуры
				pVkl_Zapr_Razrayd();
				pOtkl_Test_Razrayd();																							// Отключаем разряд
				stat4[iMUK_ZRU] |= br2;
				PauseOn(20);
	...
	case st_t_2_05:					
				...
				((stat4[iMUK_ZRU2] & br2) || ( stat4[iMUK_ZRU3] & br2)))
		{
			if (aI_razr > aIporog)	{																				// Сообщение "Не отключился разряд"=1
					...
					StepAlgortmTest = st_t_InitEnd_Alg_TVC;													// ******** Окончание ТВЦ
			}
			else	{
				stat2[iMUK_ZRU] &= ~errNoOtklRazr;						
				PauseOn(5);
				stat4[iMUK_ZRU] &= ~br3;																			
				StepAlgortmTest = st_t_2_06;
			}	
		}	
		break;
	 
	// ========== Проверка температуры АБ ...................................................................................
	case st_t_2_06:
			
		if (!bPauza) {																										
			if	( (T <= 30)||																								
						(( stat4[iMUK_ZRU2] & br3 ) && ( stat4[iMUK_ZRU3] & br3 )) ) 		
			{								
				stat4[iMUK_ZRU] |= br3;				
				stat3[iMUK_ZRU] &= ~errPrevDopustT;				
				StepAlgortmTest = st_t_2_07;
		  }
			else	{	
				PauseOn(5);
				StepAlgortmTest = st_t_2_06;	}
	...
	// ========== Синхронизация и Включение тестового разряда ...............................................................
	case st_t_2_07:
	...
	case st_t_4_02:	
		...
		if (!bPauza)
		{
			...
			if	(stat1[iMUK_ZRU] & bPC)	{	
				...
				PauseOn(20);
				StepAlgortmTest = st_t_4_03;																		
			}	
			else	{																										
				stat2[iMUK_ZRU] |= errNoVklRS;																	// Собщение "Не включается РС"=1
				PauseOn(60);
				StepAlgortmTest = st_t_5_01;																					
			}		
		}
		break;
		
	// ========== Разряд АБ с РС не более 30 час ............................................................................
	case st_t_4_03:				
																																				
		if (!bPauza) {
			if ((tstatTVC/60 >= tRazr)||																			// Если время разряда достигло предела
	...
	case st_t_4_04:
			 ...
			 ) 
		{
			 ...
			 PauseOn(20);
			 StepAlgortmTest = st_t_4_05;																			
		}	
		break;

		// .......... Ожидание отключения РС ..................................................................................		
	case st_t_4_05:							

		if (!bPauza) {
			if	(!stat1[iMUK_ZRU] & bPC)		
				stat2[iMUK_ZRU] |= errNoOtklRS;																	// Собщение "Не отключается РС"
			PauseOn(60);
			StepAlgortmTest = st_t_5_01;																						
		}
		break;
		
	// ========== Включение КОМП заряда АБ  .................................................................................
	case st_t_5_01:			
...
//		break;
	...
	case st_t_6_02:						
		...
		if (!bPauza) {
			...
			pOtkl_KOMP();																											// Откл КОМП
			PauseOn(20);
			StepAlgortmTest = st_t_6_03;
		}	
		break;
	...
	case st_t_6_03:						
		...
		if (!bPauza) {
			stat2[iMUK_ZRU] &= ~errNoVklCompZar;															// Сообщение "Не включился КОМП заряд"=0
			if (aI_zar <= aIporog)		
				stat2[iMUK_ZRU] |= errNoOtklCompZar;														// Сообщение "Не отключился КОМП заряд"=1			
			stat4[iMUK_ZRU] &= ~br7; 
			StepAlgortmTest = st_t_6_04;
		}	
		break;
		
	// ========== Заряжаем батарею максимальным током .......................................................................
	case st_t_6_04:			
	...
	case st_t_6_05:
		...
		if (!bPauza) {
			if ( ((stat4[iMUK_ZRU2] & br7)||																	
					  (stat4[iMUK_ZRU3] & br7)) ) 
				 StepAlgortmTest = st_t_7_01;																			 
		}	
		break;				
...
//		
	...
	case st_t_7_02:						
		...
		if (!bPauza) {
			...
			stat2[iMUK_ZRU] &= ~errNoOtklCompZar;															// Сообщение "Не отключился КОМП заряд"=0
			PauseOn(20);
			StepAlgortmTest = st_t_7_03;																			// Переход на проверку включения КОМП
		}
		break;
	...
	case st_t_7_03:						
		...
		if (!bPauza)		{
			if (aI_zar > aIkomp)	{																						
					...
					stat4[iMUK_ZRU] &= ~br8;
					PauseOn(10);
					StepAlgortmTest = st_t_7_04;
			}	
		}	
		break;
		
	// ========== Заряжаем батарею компенсационным током "КОМП" .............................................................
	case st_t_7_04:			

		if (!bPauza) {
			if (((tstatTVC/60 >= tVir)||(T >= Tk))||													// tstatTVC в секундах, tVir в минутах
					((stat4[iMUK_ZRU2] & br8)&&
					( stat4[iMUK_ZRU3] & br8))
				)
			{
				stat4[iMUK_ZRU] |= br8;
				StepAlgortmTest = st_t_7_05;
			}
			else	{
				PauseOn(5); 
				StepAlgortmTest = st_t_7_04;																			 
...
// ========== Начало этапа ................................................................................................
	...
	case st_t_8_12:
		...
		if (( stat5[iMUK_ZRU2] & br12 ) || ( stat5[iMUK_ZRU3] & br12 )) //если хотя бы в одном из двух других МК есть флаг синхронизации
			StepAlgortmTest = st_t_9_01;
		break;				
	
	// ========== Принудительный заряд до рабочей уставки P2 ................................................................
	case st_t_9_01:
	...
	case st_t_9_02:		
		...
		if (!bPauza) {
			if	(aI_zar > aIporog)	{	
				PauseOn(20);
				StepAlgortmTest = st_t_9_03;
			}
			else	{									
				stat2[iMUK_ZRU] |= errNoVklZar;						// Собщение "Не включился Заряд АБ"=1
				StepAlgortmTest = st_t_InitEnd_Alg_TVC;
			}	
		}
		break;
...
{	
	...
	// .......... Не включился компенсационный ток ..........................................................................
	case st_p_12:							
		if (!bPauza) {
...
//-------------------------------------------------------------------------------------------------------------------------
// Мажоритируем уставки
unsigned int MajorUst ()
{
	unsigned int tmp, i;
	unsigned int m[3];
	
	//сначала скопируем значения уставок в служебный массив
	for(i= 0; i < 3; i++)
		m[i] = iUst_mas[i];
	
	//результатом мажоритара трех разных чисел будет являться его серединное значение (Медиа́на)
	//в случае трех чисел нужно найти среднее из них
	//сортируем служебный массив по возрастанию
	if (m[0] > m[1]) { tmp = m[0]; m[0] = m[1]; m[1] = tmp; }
	if (m[1] > m[2]) { tmp = m[1]; m[1] = m[2]; m[2] = tmp; }
	if (m[0] > m[1]) { tmp = m[0]; m[0] = m[1]; m[1] = tmp; }
	
	return m[1]; //возвращаем среднее из них
}

//-------------------------------------------------------------------------------------------------------------------------
void MakePack1(void)	// Заполнение пакета телем. данными	
{		
	unsigned int tmp, ust_mag;
/*
...
*/	
	...
	iUst_mas[iMUK_ZRU] = iUst; //перед мажоритаром уставок МК ЗРУ необходимо записать уставку этого МК ЗРУ 
	ust_mag = MajorUst();

	tmp = MajorStatZRU(stat1);
	PackRs1[12] = (tmp & 0x3F) | ((ust_mag+1)<<6);	//мажоритированная уставка 9 байт
	PackRs1[13] = tmp>>8;														// 10 байт
...
{	int cntBadAk=0;																												// x = x’ ∙ z + x0, c
	...
	tVir 	 	 = (*(p_InPack+18)*z_p5[18]+ x0_p5[18]);											// tВЫР				20	Время (в минутах) выравн давл в АК в АБ в режиме ТВЦ при комп заряде
	...
	tRazr 	 = (*(p_InPack+22)*z_p5[22]+ x0_p5[22]);											// tРАЗ				24	Время разр АБ в реж ТВЦ на разрядное сопротивление (в минутах)
...
{	float add = 0.5;																											// x = x’ ∙ z + x0
	...
	PackRs6[22]  = (tVir	 - x0_p5[18])/z_p5[18] + add;										// Время выравнивания заряженности НВАБ при ТВЦ, в минутах  
	...
	PackRs6[26]  = (tRazr	 - x0_p5[22])/z_p5[22] + add;										// Время разряда на разрядное сопротивление, минуты (10-50)
...
{		
	...
	PackRs8[6] = CreateByteFromParam(StepAlgortmTest, 0, 1);
	PackRs8[7] = CreateByteFromParam(StepAlgortmPodzar, 0, 1);
	
	PackRs8[8] = 	(mk_zru_con[0]<<0) |	// Состояние связи с МК1 ЗРУ
								(mk_zru_con[1]<<1) |	// Состояние связи с МК2 ЗРУ
								(mk_zru_con[2]<<2);		// Состояние связи с МК3 ЗРУ
...
{		
	...
	stat4[iMUK_ZRU] = 0;	
	stat5[iMUK_ZRU] = 0;
	LimsCountTVC = vsCount5;		sCountTVC=0;		bPauza_TVC=1; 					// Подготовавливаем паузу 5 секунд			
	
	switch (ETVC)	{						
		case 	1:	StepAlgortmTest = st_t_InitTest;	
							LimsCountTVC = 0;																				// Пауза 0
							break;
		case	2:	StepAlgortmTest = st_t_2_01;		
							break;
		case 	3:	StepAlgortmTest = st_t_3_01; 														// Полет, идем на проверку температуры
							break;
		case 	4:	StepAlgortmTest = st_t_3_03;														// StepAlgortm = bVkl_RS;
							stat4[iMUK_ZRU] |= br5 ; 																// для синхронизации перед вкл РС	
							break;
		case	5:	StepAlgortmTest = st_t_5_01;														// StepAlgortm = bVklKomp;				
							break;
		case	6:	StepAlgortmTest = st_t_6_01;														// StepAlgortm = bTest_Vosst_et6_command;		//идем на вспомогательный шаг, так как нужно сначала подать команды
							break;
		case	7:	StepAlgortmTest = st_t_7_01;														// StepAlgortm = bTest_Vosst_et7_command;		//идем на вспомогательный шаг, так как нужно сначала подать команды		
							break;
		case	8:	StepAlgortmTest = st_t_8_01;														// StepAlgortm = bVkl_Test_Razr;		
							break;
		case	9:	StepAlgortmTest = st_t_9_01;														// StepAlgortm = bVkl_Tst_Zarayd9;
							break;
	...
	ResetAvars();																												// сброс всех аварийных сообщений
}
...
void Zaryd_NVAB_noCAN (void)											/* _З_А_Р_Я_Д___Н_В_А_Б_ по проводным командам */
{
	...
	// .......... Отключение КОМП ...Анализ ЗЗП .............................................................................
	case st_z_1:
		iUst = 1;
		pOtkl_KOMP();

		if	(!ZaprZarProv)	{																								// ЗЗП = 0 Включение заряда
			pOtkl_Zapr_Zarayd ();																							// Запрет заряда = 0
			PauseOn(5);
			StepAlgortmZar = st_z_2;
		}	
		else	{																															// ЗЗП = 1 Ожинание снятия Запрета Заряда
			pVkl_Zapr_Zarayd ();																							// Запрет заряда = 1
			PauseOn(20);
			StepAlgortmZar = st_z_3;
		}
	...
	case st_z_2:																													// ОтклЗапЗар
		if (!bPauza) {
			if (aI_zar <= aIzard) stat3[iMUK_ZRU] &= ~errNoOgrTokZar;					// Собщение "Не ограничен ток заряда"=0, aIzard = 18
			else									stat3[iMUK_ZRU] |= errNoOgrTokZar;					// Собщение "Не ограничен ток заряда"=1
			StepAlgortmZar = st_z_1;																					// След шаг алгоритма
		}
		break;

	// .......... Контроль тока с пороговым значением .......................................................................
	case st_z_3:		
		if (!bPauza) {
			if	(aI_zar > aIporog)	stat2[iMUK_ZRU] |= errNoOtklZar;					// Собщение "Не отключился Заряд АБ"=1,	aIporog	= 2
			else 										stat2[iMUK_ZRU] &= ~errNoOtklZar;					// Собщение "Не отключился Заряд АБ"=0
			StepAlgortmZar = st_z_1;																					// След шаг алгоритма Ожидание нормализации температуры
		}
		break;
...
{	int i;
	...
	//......................................................................
	//Анализ связи с двумя другими МК ЗРУ
	for (i=0;i<3;i++)	{																										
		if(i != iMUK_ZRU) //если это не наш МК, а один из двух других
		{
			if (cntSec_noMK_ZRU[i] > 4)	//если прошло достаточно секунд ожидания связи
			{				
				//связь этого МК с другими МК ЗРУ
				mk_zru_con[i] = 1; //1 - нет связи
				
				//то считаем, что у этого МК ЗРУ нет связи с МК БЭ, потому что мы не можем получить эту информацию от этого МК ЗРУ
				mk_be_osn[i] = 1; 
				mk_be_res[i] = 1;
				
				//для мажоритирования уставок считаем, что уставка МК ЗРУ, с которым нет связи, совпадает с нашей
				iUst_mas[i] = iUst;
			}	
			else	
			{			
				mk_zru_con[i] = 0; //если счетчик не досчитал до критической отметки, то считаем, что связь с соответствующим МК есть
				cntSec_noMK_ZRU[i]++;		//увеличиваем счетчик секунд
			}
		}
		else
		{
				mk_zru_con[i] = 0; //если это наш МК, то считаем, что с ним есть связь
		}
	}	
...
//	Ports_Init_Tst(); 																									// Инициализация портов для отладочной платы
	...
	while (1)												
	{
		...
		//МК ЗРУ больше не подтверждают прием пакетов телеметрии от МК БЭ, поэтому функции CAN_SendConf_1 не нужны.
		//Однако, именно благодаря им БЭ узнает, включен ли силовой канал в ЗРУ, а это необходимо, чтобы 
		//корректировать показания АК. Поэтому пока оставляем
		...
		{		
			...
			case TEST:																												// Запуск подпрограммы ТВЦ АБ (определение ёмкости АБ) 
				if (!bPauza_TVC) 																								// то контролируем окончание задержки, прежде чем перейти в ТВЦ
					Test_NVAB();
				break;
		...
		//..............................................................................................................................
		//проводные запреты
		pNotCan(); //считали состояние проводных линий bitNotZar, bitNotRaz
		...
		if (tstProvod)	{
			...
			if(bitNotZar != Prev_bitNotZar) //если изменилось состояние по сравнению с предыдущим
			{
				...
				if (cntIzmZar > 2)	ZaprZarProv = 1; //запрет заряда проводной = 1
				else								ZaprZarProv = 0; //запрет заряда проводной = 0
				cntIzmZar = 0;
			}
			...
			if(bitNotRaz != Prev_bitNotRaz) //если изменилось состояние по сравнению с предыдущим
			{
				...
				if (cntIzmRaz > 2)	ZaprRazrProv = 1; //запрет разряда проводной = 1
				else								ZaprRazrProv = 0; //запрет разряда проводной = 0
				cntIzmRaz = 0;
			}
