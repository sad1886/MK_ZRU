...
{		int ii;
	...
	case bWeitMKs:                            ////////////  Алгоритм Б

		stat4[iMUK_ZRU] |= bready7;																					// Установили флаг Прекращаем заряд
   	if ((stat4[iMUK_ZRU2] & bready8)&&			////////////														
			 ( stat4[iMUK_ZRU3] & bready8))	{			////////////		
				...
				if (((stat4[iMUK_ZRU2] & bready7) ||																	// Заряд завершён
			      ( stat4[iMUK_ZRU3] & bready7)) ||
				    ...
				    ( stat3[iMUK_ZRU3] & bZaprZar))
			         ...
			         stat3[iMUK_ZRU] |= errPrevDopustT;																// "Превышение допустимой температуры НВАБ"
			         StepAlgortm = bInitEnd_Alg_TVC;																		// ******** Окончание по превышению температуры																				// Ожидание завершения заряда на других МК
			  }
			    else	{
				StepAlgortm = bWeitMKs;																				// Ожидание завершения заряда на других МК
			}	
...
//		StepAlgortm = bWaitOtkl_Zarayd;									
		StepAlgortm = bTest_Endtemp;									

	break;
	
	// .......... Ожидание Отключения заряда ................................................................................9
	case bTest_Endtemp:			// Алгоритм А
	////////////
		if ((stat4[iMUK_ZRU2] & bready8)&&			////////////														
			 ( stat4[iMUK_ZRU3] & bready8))	{		////////////		
			 if  ((stat3[iMUK_ZRU2] & bZaprZar)||																	// включен запрет Заряда
...
{	float tHour;
	...
	if (statTVC)	{
		...
		LimsCount = vsCount20;	sCountTVC=0;		bPauza_TVC=1;										// Активация паузы 20 сек
		//stat3[iMUK_ZRU] = 0;
	}
}
...
//					 (stat1[iMUK_ZRU] & bZaryad))	stat2[iMUK_ZRU] |= errNoVklZar;	// Собщение "Не включился Заряд АБ"=1
	...
	case bTst_T_NVAB:																											// ОтклЗапЗар
		...
		if	((mode_Zaryad)&&(T >= T_max)) 	{																									// Отключаем заряд. Вкл_КОМП = ОТКЛ ЗАРЯД
	...
	case bWaitTmax_2:
		...
		if (!bPauza) {
			if	(T <= T_max-2)	{																							// Отключаем заряд. Вкл_КОМП = ОТКЛ ЗАРЯД
				...
				// Если идет процесс разряда то снимать запрет заряда можно только когда АБ остынет 
			//	if (mode_Razryad)		StepAlgortmZar = bInitZarayd;		//
			//  else {																// "Петля" - процесс охлаждения
				  StepAlgortmZar = bWaitTmax_2;														      	// След шаг алгоритма
				  LimsCount = vsCount20;	sCount=0;		bPauza=1;										// Активация паузы 20сек
			// }	
		 }
	 }
		...
		if (!bPauza_R) {																										// Пауза 5 сек
			...
			if	((Uab <= 72)||(Umin_ak <= 0.2))	{	
				...
				if	(T >= 50)	
					...
					if (mode_Razryad)	 stat3[iMUK_ZRU] &= ~errPrevDopustT;			//				
				  else  StepAlgortmRazr = bInitRazryda;
